...
function menu($user) {
	...
	if($user !== false) {
		...
		<a onclick="showNotification()"><div class="menu_btn" id="notifications_btn" title="'.$LNG['title_notifications'].'"><img src="'.$CONF['url'].'/'.$CONF['theme_url'].'/images/notification.png" id="notifications_img" /></div></a>
		<a href="'.$CONF['url'].'/index.php?a=messages"><div class="menu_btn" title="'.$LNG['title_messages'].'"><img src="'.$CONF['url'].'/'.$CONF['theme_url'].'/images/message.png" /></div></a>
		...
		<div class="notification-container">
			<div class="notification-content">
				<div class="notification-inner"><a href="'.$CONF['url'].'/index.php?a=notifications"><strong>'.$LNG['view_all_notifications'].'</strong></a> <a onclick="showNotification(\'close\')" title="'.$LNG['close_notifications'].'"><div class="delete_btn"></div></a></div>
		...
		function checkNewNotifications(x) {
			$.ajax({
				...
				success: function(html) {
					...
					if(html.indexOf("'.$LNG['no_notifications'].'") == -1 && html !== "") {
						$("#notifications_btn").html("<img src=\"'.$CONF['url'].'/'.$CONF['theme_url'].'/images/notification_new.png\" />");
						clearTimeout(checkNewNotifications);
					} else {
						setTimeout(checkNewNotifications, '.$settings['intervaln'].');
					}
			   }
			});
		}
...
class register {
	...
	public $captcha;				
	public $captcha_on;				
	public $message_privacy;		
	public $verified;				
	public $like_notification;		
	public $comment_notification;	
	...
	function process() {
		...
		if(empty($arr)) { 
			...
			foreach($arr as $err) {
				return notificationBox('transparent', $LNG['error'], $LNG["$err"], 1); // Return the error value for 
			}
		}
		return $x;		
	}
	...
	function query() {
		$query = sprintf("INSERT into `users` (`username`, `password`, `email`, `date`, `image`, `privacy`, `cover`, `verified`, `online`, `notificationl`, `notificationc`, `notificationm`, `notificationd`, `email_comment`, `email_like`) VALUES ('%s', '%s', '%s', '%s', 'default.png', '%s', 'default.png', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');", $this->db->real_escape_string(strtolower($this->username)), md5($this->db->real_escape_string($this->password)), $this->db->real_escape_string($this->email), date("Y-m-d H:i:s"), $this->db->real_escape_string($this->message_privacy), $this->db->real_escape_string($this->verified), time(), $this->db->real_escape_string($this->like_notification), $this->db->real_escape_string($this->comment_notification), $this->db->real_escape_string($this->message_notification), $this->db->real_escape_string($this->chat_notification), $this->db->real_escape_string($this->email_comment), $this->db->real_escape_string($this->email_like));
		$this->db->query($query);
...
class updateUserSettings {
	...
	function validate_inputs($data) {
		...
		if($this->verify_if_email_exists($this->id, $data['email'])) {
...
class feed {
	...
	public function profileData($username = null, $id = null) {
		...
		if($id) {
			$query = sprintf("SELECT `idu`, `username`, `email`, `first_name`, `last_name`, `location`, `website`, `bio`, `date`, `facebook`, `twitter`, `gplus`, `image`, `private`, `background`, `privacy`, `born`, `cover`, `verified`, `gender` FROM `users` WHERE `idu` = '%s'", $this->db->real_escape_string($id));
		} else {
			$query = sprintf("SELECT `idu`, `username`, `email`, `first_name`, `last_name`, `location`, `website`, `bio`, `date`, `facebook`, `twitter`, `gplus`, `image`, `private`, `background`, `privacy`, `born`, `cover`, `verified`, `gender` FROM `users` WHERE `username` = '%s'", $this->db->real_escape_string($username));
		}
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<div class="cover-image" style="background-position: center; background-image: url('.$this->url.'/thumb.php?src='.((!empty($profile['cover'])) ? $profile['cover'] : 'default.png').'&w=900&h=200&t=c)">
							</div>
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a href="'.$this->url.'/index.php?a=profile&u='.$profile['username'].'"><img src="'.$this->url.'/thumb.php?src='.((!empty($profile['image'])) ? $profile['image'] : 'default.png').'&t=a&w=150&h=150" /></a>
									</div>
								</div>
								<div class="cover-description-content">
									<div class="cover-username">'.((!empty($profile['verified'])) ? '<img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/verified.png" title="'.$LNG['verified_user'].'" />' : '').''.realName($profile['username'], $profile['first_name'], $profile['last_name']).'</div>
									<div id="subscribe'.$profile['idu'].'">'.$this->getSubscribe(null, null, null).'</div>'.$this->chatButton($profile['idu'], $profile['username'], 1).'
								</div>
							</div>
						</div>
					</div>
	...
	function fetchProfileWidget($username, $name, $image) {
		global $LNG;
		$widget =  '<div class="sidebar-container">
	...
	function fetchProfileInfo($profile) {
		global $LNG;
		$info = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['profile_about'].''.(($this->profile == $this->username) ? ' (<a href="'.$this->url.'/index.php?a=settings">'.$LNG['admin_ttl_edit'].'</a>)' : '').'</div>
		'.((!empty($profile['location'])) ? '<div class="sidebar-about">'.$LNG['profile_location'].': <strong>'.$profile['location'].'</strong></div>' : '').'
		'.(($profile['born'] !== '0000-00-00') ? '<div class="sidebar-about">'.$LNG['profile_born'].': <strong>'.date('F j, Y', strtotime($profile['born'])).'</strong></div>' : '').'
		'.((!empty($profile['gender'])) ? '<div class="sidebar-about">'.$LNG['ttl_gender'].': <strong>'.(($profile['gender'] == 1) ? $LNG['male'] : $LNG['female']).'</strong></div>' : '').'
		'.((!empty($profile['website'])) ? '<div class="sidebar-about">'.$LNG['profile_website'].': <strong><a href="'.$profile['website'].'" target="_blank" rel="nofllow">'.$LNG['profile_view_site'].'</a></strong></div>' : '').'
		'.((!empty($this->subscriptionsList[1])) ? '<div class="sidebar-about">'.$LNG['follows'].': '.$this->sidebarSubs(0, 1).'</div>' : '').'
		'.((!empty($this->subscribersList[1])) ? '<div class="sidebar-about">'.$LNG['followed_by'].': '.$this->sidebarSubs(1, 1).'</div>' : '').'
		'.((!empty($profile['facebook'])) ? '<div class="sidebar-about">Facebook: <strong><a href="http://facebook.com/'.$profile['facebook'].'" target="_blank" rel="nofllow">'.$LNG['profile_view_profile'].'</a></strong></div>' : '').'
		'.((!empty($profile['gplus'])) ? '<div class="sidebar-about">Google+: <strong><a href="http://plus.google.com/'.$profile['gplus'].'" target="_blank" rel="nofllow">'.$LNG['profile_view_profile'].'</a></strong></div>' : '').'
		'.((!empty($profile['twitter'])) ? '<div class="sidebar-about">Twitter: <strong><a href="http://twitter.com/'.$profile['twitter'].'" target="_blank" rel="nofllow">'.$LNG['profile_view_profile'].'</a></strong></div>' : '').'
		'.((!empty($profile['bio'])) ? '<div class="sidebar-about">'.$LNG['profile_bio'].': '.$profile['bio'].'</div>' : '').'
		</div></div>';
		
		return $info;
	}
	
	function checkNewNotifications($limit, $type = null, $ln = null, $cn = null, $mn = null, $dn = null) {
		global $LNG, $CONF;
		// $ln, $cn, $mn, $dn holds the filters for the notifications
		// Type 0: Just check for and show the new notification alert
		// Type 1: Return the last 3 notifications from each category.
		// Type 2: Return the latest X notifications (read and unread)
		// Check if the user has a last date when the notification was checked
		$query = sprintf("SELECT * FROM `notifications` WHERE `id` = '%s'", $this->db->real_escape_string($this->id));
		
		// Run the query
		$result = $this->db->query($query);
		
		// If there is no timestamp available, register a new user with a timestamp
		if(!$result->num_rows) {
			// If there is no timestamp available, register a new user with a timestamp
			$insert = $this->db->query(sprintf("INSERT INTO `notifications` (`id`, `time`) VALUES ('%s', '%s');", $this->db->real_escape_string($this->id), date("Y-m-d H:i:s")));
		...
		// fetch the row, [1] is the timestamp array element
		...
		// If the user hasn't subscribed to anyone, then don't go any furhter
		...
		if(!$type) {
			...
			if($ln) {
				$checkLikes = $this->db->query(sprintf("SELECT * FROM `likes`,`users` WHERE `likes`.`time` > '%s' AND `likes`.`by` = `users`.`idu` AND `likes`.`by` IN (%s) ORDER BY `id` DESC LIMIT 1", $timestamp[1], implode(',', $subscriptions)));
			...
			if($cn) {
				$checkComments = $this->db->query(sprintf("SELECT * FROM `comments`,`users` WHERE `comments`.`time` > '%s' AND `comments`.`uid` = `users`.`idu` AND `comments`.`uid` IN (%s) ORDER BY `id` DESC LIMIT 4", $timestamp[1], implode(',', $subscriptions)));
			...
			if($mn) {
				$checkMessages = $this->db->query(sprintf("SELECT * FROM `messages`,`users` WHERE `messages`.`time` > '%s' AND `messages`.`uid` = `users`.`idu` AND `messages`.`uid` IN (%s) ORDER BY `id` DESC LIMIT 1", $timestamp[1], implode(',', $subscriptions)));
			...
			// Check for new chat messages events
			if($dn) {
				$checkChats = $this->db->query(sprintf("SELECT * FROM `chat`,`users` WHERE `chat`.`to` = '%s' AND `chat`.`time` > '%s' AND `chat`.`read` = '0' AND `chat`.`from` = `users`.`idu` ORDER BY `id` DESC LIMIT 1", $this->db->real_escape_string($this->id), $timestamp[1]));
				
				// If any, return 1 (show notification)
				if($checkChats->num_rows > 0) {
					return 1;
				}
			}
		} else {
			// Update the last-time checked timestamp
			$this->db->query("UPDATE `notifications` SET `time` = '".date('Y-m-d H:i:s')."' WHERE id = '{$this->id}'");
			...
			if($type == 1) {
				...
				if($ln) {
					// Check for new likes events
					$checkLikes = $this->db->query(sprintf("SELECT * FROM `likes`,`users` WHERE `likes`.`time` > '%s' AND `likes`.`by` = `users`.`idu` AND `likes`.`by` IN (%s) ORDER BY `id` DESC LIMIT %s", $timestamp[1], implode(',', $subscriptions), $limit));
					// Fetch the comments
				...
				if($cn) {
					// Check for new comments events
					$checkComments = $this->db->query(sprintf("SELECT * FROM `comments`,`users` WHERE `comments`.`time` > '%s' AND `comments`.`uid` = `users`.`idu` AND `comments`.`uid` IN (%s) ORDER BY `id` DESC LIMIT %s", $timestamp[1], implode(',', $subscriptions), $limit));
					// Fetch the comments
				...
				if($mn) {
					// Check for new messages events
					$checkMessages = $this->db->query(sprintf("SELECT * FROM `messages`,`users` WHERE `messages`.`time` > '%s' AND `messages`.`uid` = `users`.`idu` AND `messages`.`uid` IN (%s) ORDER BY `id` DESC LIMIT %s", $timestamp[1], implode(',', $subscriptions), $limit));
					// Fetch the messages
				...
				if($dn) {
					// Check for new messages events
					$checkChats = $this->db->query(sprintf("SELECT * FROM (SELECT * FROM `chat`,`users` WHERE `chat`.`to` = '%s' AND `chat`.`time` > '%s' AND `chat`.`read` = '0' AND `chat`.`from` = `users`.`idu` ORDER BY `id` DESC) as x GROUP BY `from`", $this->db->real_escape_string($this->id), $timestamp[1]));
					// Fetch the chat
				...
				// If there are no new notifications, get the lastest notifications
				if(empty($likes) && empty($comments) && empty($messages) && empty($chats)) {
					if($ln) {
						$checkLikes = $this->db->query(sprintf("SELECT * FROM `likes`,`users` WHERE `likes`.`by` = `users`.`idu` AND `likes`.`by` IN (%s) ORDER BY `id` DESC LIMIT %s", implode(',', $subscriptions), $limit));
					...
					if($cn) {
						$checkComments = $this->db->query(sprintf("SELECT * FROM `comments`,`users` WHERE `comments`.`uid` = `users`.`idu` AND `comments`.`uid` IN (%s) ORDER BY `id` DESC LIMIT %s", implode(',', $subscriptions), $limit));
					...
					if($mn) {
						$checkMessages = $this->db->query(sprintf("SELECT * FROM `messages`,`users` WHERE `messages`.`uid` = `users`.`idu` AND `messages`.`uid` IN (%s) ORDER BY `id` DESC LIMIT %s", implode(',', $subscriptions), $limit));
					...
					if($dn) {
						// Show unread messages, even if they are older, when no new notifications are avilable
						$checkChats = $this->db->query(sprintf("SELECT * FROM (SELECT * FROM `chat`,`users` WHERE `chat`.`to` = '%s' AND `chat`.`read` = '0' AND `chat`.`from` = `users`.`idu` ORDER BY `id` DESC) as x GROUP BY `from` LIMIT %s", $this->db->real_escape_string($this->id), $limit));
			...
			$array = array_merge($likes, $comments, $messages, $chats);
			...
			foreach($array as $value) {
				...
				$events .= '<div class="notification-row"><div class="notification-padding">';
				if($value['event'] == 'like') {
					...
					$events .= '<div class="notification-image"><img class="notifications" src='.$this->url.'/thumb.php?src='.$value['image'].'&t=a&w=50&h=50" /></div><div class="notification-text">'.sprintf($LNG['new_message_notification'], $this->url.'/index.php?a=profile&u='.$value['username'], realName($value['username'], $value['first_name'], $value['last_name']), $this->url.'/index.php?a=post&m='.$value['id']).'.<br /><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/message_n.png" width="17" height="17" /><span class="timeago'.$b.'" title="'.$time.'">'.$time.'</span></div>';
				} elseif($value['event'] == 'chat') {
					$events .= '<div class="notification-image"><img class="notifications" src='.$this->url.'/thumb.php?src='.$value['image'].'&t=a&w=50&h=50" /></div><div class="notification-text">'.sprintf($LNG['new_chat_notification'], $this->url.'/index.php?a=profile&u='.$value['username'], realName($value['username'], $value['first_name'], $value['last_name']), $this->url.'/index.php?a=messages&u='.$value['username'].'&id='.$value['idu']).'.<br /><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/chat_n.png" width="17" height="17" /><span class="timeago'.$b.'" title="'.$time.'">'.$time.'</span></div>';
				}
				$events .= '</div></div>';
			}
			$events .= '<div class="notification-row"><div class="notification-padding"><a href="'.$this->url.'/index.php?a=settings&b=notifications">'.$LNG['notifications_settings'].'</a></div></div>';
			return $events;
		}
	...
	function getSubscribe($type = null, $list = null, $z = null) {
		...
		} elseif($this->username == $profile['username']) {
		...
		if($result = $this->db->query($query)) {
			...
			foreach($rows as $comment) {
				...
				<div class="message-reply-container" id="comment'.$comment['id'].'">
					'.$delete.'
					<div class="message-reply-avatar">
						<a href="'.$this->url.'/index.php?a=profile&u='.$comment['username'].'"><img src="'.$this->url.'/thumb.php?src='.$comment['image'].'&t=a" /></a>
					</div>
	...
	function getType($type, $value, $id) {
		...
		switch($type) {
			...
			// If it's a map
			case "map":
				return '<div class="message-type-map"><img src="https://maps.googleapis.com/maps/api/staticmap?center='.$value.'&zoom=13&size=700x150&maptype=roadmap&markers=color:red%7C'.$value.'&sensor=false&scale=2" /></div>
				<div class="message-divider"></div>';
				break;
			...
			case "picture":
				...
				if(count($images) == 1) {
					...
					foreach($images as $image) {
						$result .= '<a onclick="gallery(\''.$image.'\', '.$id.')" id="'.$image.'"><img src="'.$this->url.'/thumb.php?src='.$image.'&w=650&h=300&t=m" /></a>';
						$i++;
					}
				} else {
					...
					$i = 0;
					foreach($images as $image) {
						$result .= '<a onclick="gallery(\''.$image.'\', '.$id.')" id="'.$image.'"><div class="image-thumbnail-container"><div class="image-thumbnail"><img src="'.$this->url.'/thumb.php?src='.$image.'&w=204&h=204&t=m" /></div></div></a>';
						$i++;
					}
					$result .= '</div>';
				}
	...
	function sidebarGender($bold) {
		...
		$link = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['filter_gender'].'</div>';
	...
	function sidebarNotifications($bold) {
		...
		// Start the output
		$row = array('likes', 'comments', 'messages', 'chats');
		$link = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['filter_events'].'</div>';
	...
	function sidebarDates($bold, $values = null) {
		...
		if($row) {
			// Start the output
			$link = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['archive'].'</div>';
			$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].$profile.'">'.$LNG["all_time"].'</a></div>';
			foreach($row as $date) {
				$converted = strtotime(wordwrap($date, 4, '-', true));
				// Start the strong tag
				if($date == $bold) {
					$link .= '<strong>';
				}
				...
				// Output the links
				$link .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a='.$_GET['a'].$profile.'&filter='.$date.'">'.date('F - Y', $converted).'</a></div>';
	...
	function sidebarSubs($type, $for) {
		...
		if($result[1] > 0) {
			if($for == 0) {
				$i = 0;
				$output = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header"><a href="'.$this->url.'/index.php?a=profile&u='.((!empty($this->profile)) ? $this->profile : $this->username).'&r='.$r.'">'.$title.' <span class="sidebar-header-light">('.$result[1].')</span></a></div>';
	...
	function onlineUsers($type = null, $value = null) {
		...
		// Type 0: Display the friends for feed/subscriptions page
		...
		if($type == 1) {
			// Output the users
			$output = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header"><input type="text" placeholder="'.$LNG['search_for_users'].'"  id="search-list" /></div><div class="search-list-container"></div><div class="sidebar-list">';
			if(!empty($rows)) {
				$i = 0;
				foreach($rows as $row) {
			...
			if(!empty($rows)) {
				...
				$output = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header"><a href="'.$this->url.'/index.php?a=messages">'.$LNG['online_friends'].' <span class="sidebar-header-light">('.$query->num_rows.')</span></a></div>';
	...
	function getChatMessages($uid, $cid, $start, $type = null) {
		...
		if($result = $this->db->query($query)) {
			...
			if($type !== 1) {
				...
				$update = $this->db->query(sprintf("UPDATE `chat` SET `read` = '1', `time` = `time` WHERE `from` = '%s' AND `to` = '%s'", $this->db->real_escape_string($uid), $this->db->real_escape_string($this->id)));
			}
	...
	function sidebarPlaces($id) {
		...
		if(!empty($rows)) {
			$i = 0;
			$output = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header"><a href="'.$this->url.'/index.php?a=profile&u='.((!
			...
			foreach($rows as $row) {
				...
				$output .= '<div class="sidebar-subscriptions"><div class="sidebar-title-container"><div class="sidebar-places-name">'.$row['value'].'</div></div><a href="'.$this->url.'/index.php?a=post&m='.$row['id'].'"><img src="https://maps.googleapis.com/maps/api/staticmap?center='.$row['value'].'&zoom=13&size=150x150&maptype=roadmap&sensor=false&scale=2" /></a></div>';
	...
	function sidebarSuggestions() {
		...
		if($this->getSubscriptionsList($this->id)) {
			...
			$query = $this->db->query(sprintf("SELECT *, COUNT(`subscriber`) AS popular FROM `relations`, `users` WHERE `relations`.`leader` = `users`.`idu` AND `relations`.`leader` NOT IN (%s) GROUP BY `leader` ORDER BY popular DESC LIMIT 6", $this->id.','.$this->db->real_escape_string($this->getSubscriptionsList($this->id))));
		} else {
			$query = $this->db->query("SELECT *, COUNT(`subscriber`) AS popular FROM `relations`, `users` WHERE `relations`.`leader` = `users`.`idu` GROUP BY `leader` ORDER BY popular DESC LIMIT 6");
		}
		...
		if(!empty($rows)) {
			...
			$output = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['sidebar_suggestions'].'</div>';
			foreach($rows as $row) {
	...
	function sidebarTrending($bold, $per_page) {
		...
		if(!empty($hashtags)) {
			...
			$count = array_count_values(array_filter($hashtags));
			
			// Sort them by trend
			arsort($count);
			$output = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['sidebar_trending'].'</div>';
	...
	function getSearch($start, $per_page, $value, $filter = null, $type = null) {
		// $type - switches the type for live search or static one
		global $LNG;
		...
		if($filter == 'm' || $filter == 'f') {
			...
			$query = $this->db->query(sprintf("SELECT * FROM `users` WHERE `gender` = '%s' AND (`username` LIKE '%s' OR concat_ws(' ', `first_name`, `last_name`)  LIKE '%s') ORDER BY `idu` DESC LIMIT %s, %s", $gender, '%'.$this->db->real_escape_string($value).'%', '%'.$this->db->real_escape_string($value).'%', $this->db->real_escape_string($start), ($per_page + 1)));
		} 
		// If the filter is a date range (digit type)
		elseif(ctype_digit($filter)) {
			// HERE COMES THE FILTER FOR AGE BETWEEN X & Y
		} else {
			$query = $this->db->query(sprintf("SELECT * FROM `users` WHERE `username` LIKE '%s' OR concat_ws(' ', `first_name`, `last_name`) LIKE '%s' ORDER BY `idu` DESC LIMIT %s, %s", '%'.$this->db->real_escape_string($value).'%', '%'.$this->db->real_escape_string($value).'%', $this->db->real_escape_string($start), ($per_page + 1)));
		}
								...
								<div id="subscribe'.$row['idu'].'">'.$this->getSubscribe(0, array('idu' => $row['idu'], 'username' => $row['username'], 'private' => $row['private']), 1).'</div>'.$this->chatButton($row['idu'], $row['username'], 1).'
									...
									<div class="message-top">
										<div class="message-author" id="author'.$row['idu'].'">
											<a href="'.$this->url.'/index.php?a=profile&u='.$row['username'].'">'.$row['username'].'</a>
										</div>
										<div class="message-time">
	...
	function validateMessage($message, $image, $type, $value, $privacy) {
			...
			if(empty($type)) {
				...
				if(in_array($type, $allowedType)) {
					if($type == 'video') {
						...
						} elseif(substr($value, 0, 17) == "http://vimeo.com/" || substr($value, 0, 21) == "http://www.vimeo.com/" || substr($value, 0, 14) == "www.vimeo.com/" || substr($value, 0, 10) == "vimeo.com/") {
		...
		if($error) {
			...
			// Escape thge message
			$message = $this->db->real_escape_string(htmlspecialchars($message));
...
function generateAd($content) {
	...
	$ad = '<div class="sidebar-container"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['sponsored'].'</div>'.$content.'</div></div>';
	return $ad;
}
